...
4. 그날 TODO 전체가 완료됐는지 확인. 완료라면:
   a. 연속 계산: last_complete_date가 "어제(논리적)"면 N = current_streak+1,
      아니면 N = 1. (streak_reset_days 설정 시 N > 설정값이면 N=1로 리셋)
   b. 완주 보너스 +10, 연속 보너스 +10×(N-1)을 A·B 각각 지급(원장 기록 포함).
   c. users.current_streak = N, last_complete_date = 오늘.
   d. daily_bonus_log에 (오늘, 10, 10×(N-1), N) 기록.
5. 뱃지 조건 검사(아래 4.4). 새 뱃지는 user_badges INSERT.
6. 응답 payload 구성.
COMMIT
```
...
4. daily_bonus_log에 오늘 행이 있으면 (= 완주 보너스를 받았었다면):
   a. 기록된 day_bonus, streak_bonus를 각각 음수로 원장 기록 + 잔액 회수.
   b. current_streak = streak_n - 1,
      last_complete_date = (streak_n-1 > 0 ? 어제 : NULL) 로 되돌림.
   c. daily_bonus_log 행 삭제.
COMMIT
```
...
1. 미션 완료 → A·B 동시 +5, 원장 2행 생성.
2. 마지막 TODO 완료 → 완주+연속 보너스 정확 지급 (N=1,2,3 각각).
3. 토글 OFF → 보너스 포함 정확 회수, streak 되돌림.
